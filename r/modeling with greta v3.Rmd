...
for this version we will perform logistic regression to differentiate between the rich contracts (aav>5) and the not so rich contracts (aav<=5).


# logistic regression to predict if aav>=5 or aav<5
...
train[, groups:=ifelse(aav<5, 0, 1)]
test[, groups:=ifelse(aav<5, 0, 1)]
...
```{r}
draws = greta::mcmc(m, n_samples = 15000, n_cores=4, chains=2,
...
#train[,groups:=ifelse(GPerSeason<20, 1, ifelse(GSPerSeason/GPerSeason<.5,2,3))]
...
#test[,groups:=ifelse(GPerSeason<20, 1, ifelse(GSPerSeason/GPerSeason<.5,2,3))]
#test$MPandGPS=NULL
test1 = test[groups==1, ]
...
# regression model 1 for the players predicted to have aav>=5
...
```{r}
ypred = calculate(ex_mup, values=draws, trace_batch_size = 250, nsim=1500)
pred = colMeans(ypred$ex_mup)
pred = min_max_transform(method=y_norm1, data=pred, inverse=TRUE)
#pred = predict(y_norm, newdata=pred, inverse=TRUE)
...
# regression model 2 for players predicted to have aav <5
...
y_norm2 =min_max_transform(Second_group$y, hard_max = .4*112, hard_min = 0)
...
#min_trunc = predict(y_norm, newdata=.125)
mu = ilogit(mu) # beta was pretty good but gamma even better
distribution(greta_y) = gamma(sigma*mu, sigma*(1-mu))#normal(mu, sigma)
yTest_normalized = min_max_transform(method=y_norm2,
                                     predict=TRUE,
                                     data=Second_group$yTest)
...
```{r}
# get predictionss
ypred2 = calculate(mu, values=draws2, trace_batch_size = 250, nsim=1000)
pred2 = colMeans(ypred2$mu)
pred2 = min_max_transform(method=y_norm2, data=pred2, inverse=TRUE)
#pred2 = predict(y_norm, newdata = pred2, inverse = TRUE)
pred2 = ifelse(pred2>max(train0$aav), max(train0$aav), pred2)
pred2 = data.frame(pred=pred2, obs=Second_group$y)
caret::postResample(pred=pred2, obs=Second_group$y)
```
