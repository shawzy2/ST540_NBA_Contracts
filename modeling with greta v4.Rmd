...
#             )]
boxscore_stats = c('MP', 'FG', '3P', 'FT', 'REB', 'AST', 'TOV', 'STL', 'BLK', 'PF', 'PTS', 'BPM')
boxscore_cols = c()
for(i in boxscore_stats){
boxscore_cols = c( boxscore_cols, colnames(train)[str_detect(colnames(train), str_c(i, '$', sep=''))])
}
cols_to_keep = c(boxscore_cols, colnames(train)[1:34])
train = train[,colnames(train) %in% cols_to_keep, with=FALSE]
# for(i in 1:length(boxscore_cols)){
# 	for(j in 1:length(boxscore_cols)){
# 		if(i<j){
# 	
# 			col = str_c(boxscore_cols[i], 'and',boxscore_cols[j],sep='')
# 			train[,col] = train[,boxscore_cols[i], with=FALSE] * train[,boxscore_cols[j], with=FALSE]
# }
# 
# }
# }
...
# stats_for_interaction_effect =  colnames(train)[str_which(colnames(train), '^WA')]
# for(i in team_names){
...
draws = greta::mcmc(m, n_samples = 20000, n_cores=4, chains=2,
                    warmup=10000, verbose=TRUE, thin = 5)
#effectiveSize(draws)
#geweke.diag(draws[[1]])
```
...
```{r}
draws = greta::mcmc(m, n_samples = 15000, n_cores=4, chains=2, thin = 5,
                    warmup=5000, verbose=TRUE)
```
...
coefs = results$quantiles[1:(nvar), ]
...
```{r model2}
draws2 = greta::mcmc(m, n_samples = 50000, n_cores=4, chains=2, thin = 15,
                    warmup=20000, verbose=TRUE)
...
```{r model3}
draws3 = greta::mcmc(m, n_samples = 50000, n_cores=4, chains=2, thin = 15,
                    warmup=20000, verbose=TRUE)
...
mcmc_trace(draws3[[1]][,30:60], facet_args = list(ncol=3))
```
...
```{r}
select_cols = colnames(draws3[[1]])[str_detect(colnames(draws3[[1]]), '^mu')]
select_cols = c(select_cols, 'sigma')
like = rbind(draws3[[1]][,select_cols], draws3[[2]][, select_cols])
...
plot(pred_total$obs, pred_total$residuals,
     xlab = 'Observed aav', ylab = 'Residuals (observed - predicted', main = 'Residuals by observed aav')
plot(pred_total$obs, pred_total$pred,xlab = 'Observed aav', ylab = 'Residuals (observed - predicted', main = 'Residuals by observed aav')
```
...
```{r}
rowmins = apply(ypred$ex_mup,1, min)
rowmins = predict(y_norm3, newdata=rowmins, inverse=TRUE)
rowmaxes = apply(ypred$ex_mup, 1, max)
